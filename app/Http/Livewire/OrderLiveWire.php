...
class OrderLiveWire extends Component
{
    ...
    public function addToCart(int $productId)
    {
        ...
        $isMarketClosed = $article->market->name;
        ...
        if (count($articleInCart) >= 1 && $isActive && $isMarketClosed) {
            ...
            if ($isActive && !$isMarketClosed || auth()->user()->superUser && $isActive) {
                ShoppingCart::add($productId, $article->name, 1, $article->price, ['color' => $article->color, 'image' => $image, 'isActive' => $article->isActive, 'marketId' => $article->marketId, 'market' => $article->market->name, 'shipping' => $article->market->orderPaid]);
            }
        }
    }
    ...
    public function repeatOrder(){
        ...
        if($order) {
            foreach ($order->orderproduct as $item) {
                ...
                $isMarketClosed = $item->product->first()->market->name;
                $image = count($item->product->first()->images) > 0 ? $item->product->first()->images[0]->url : 'https://dummyimage.com/400x400';
                if ($isActive && !$isMarketClosed || auth()->user()->superUser && $isActive) {
                    ShoppingCart::add($item->product_id, $item->product->first()->name, $item->quantity, $item->product->first()->price, ['color' => $item->product->first()->color, 'image' => $image, 'isActive' => $item->product->first()->isActive, 'marketId' => $item->product->first()->marketId, 'market' => $item->product->first()->market->name, 'shipping' => $item->product->first()->market->orderPaid]);
                }
            }
